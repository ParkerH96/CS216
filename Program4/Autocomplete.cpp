...
begin = clock();
// your code to time
   for(int i = 0; i < terms.size(); i++){ //loop through every element in the terms list
	if(terms[i].query.substr(0, prefix.length()) == prefix){ //find all matches
		my_vector.push_back(terms[i]); //push them back in the matches vector
	}
   }
end = clock();
double elaspsed_secs = double(end - begin) / 60;
cout << "!!! " << elaspsed_secs << endl;
   for(int j = 0; j < my_vector.size(); j++){	//bubble sort the smaller, matches vector
      for(int i = 0; i < my_vector.size() - 1; i++){
         if(my_vector[i].weight < my_vector[i+1].weight){
	    swap(my_vector[i].weight, my_vector[i+1].weight); //swap them if the first one is smaller
         }
      }
   }
